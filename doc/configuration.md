...
- `render_mode`：监控显示模式，支持 `auto`、`live`、`plain`、`web`
  - `auto`：自动检测终端能力，终端环境下使用 live 渲染（同时启动 web 仪表盘），非终端环境使用 web 模式
  - `live`：强制使用 Rich Live 终端渲染（不启动 web 仪表盘，打开页面会看到Web mode is not enabled的提示）
  - `plain`：普通文本输出，便于日志收集
  - `web`：网页仪表盘模式，通过浏览器访问监控界面，支持实时数据推送、图表、节点拓扑可视化，终端安静
